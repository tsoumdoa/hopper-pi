...
| Tool                   | Parameters                                                                                                                    | What it does                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gh_get_canvas`        | `subgraph?`, `selectionOnly?`                                                                                                 | MANDATORY: place ALL components before calling. Call once after all are placed to get GUIDs for wiring and verify the build. Do NOT call before placement or between zones. Use `selectionOnly: true` to inspect only objects the user has selected on the canvas (for targeted edits; does not replace the full post-placement read).                                                                                  |
| `gh_list_components`   | `queries[]`, `searchFrom?`, `limit?`, `offset?` | Searches Grasshopper component types by keyword with multi-token ranking (whitespace and camelCase shorthand). Returns ranked matches grouped by category/subcategory with typeGuids for `gh_edit_components`. `searchFrom`: `"vanilla"` (default), `"plugin"`, or `"params"`. Prefer one component per query string; paginate with limit/offset if needed. |
...
| `gh_create_widget`   | slider, panel, toggle, swatch, scribble, valueList                                                                                                     | Create Grasshopper UI widgets at a canvas position. Panel requires `textOutput`: `"singleString"` or `"oneItemPerLine"` (see [data-type-guide.md](./mds/reference/data-type-guide.md)).                                                                                                                                            |
| `gh_mutate_widget`   | slider: `setValue`, `setRange`; panel: `setText`, `setProperty`; toggle: `setValue`; swatch: `setColor`; scribble: `setText`; valueList: `setSelected` | Modify existing widgets by targetId. Panel `setProperty` also requires `textOutput`.                                                                                                                                                                                 |
...
2. Sends `{ type: "getCurrentCanvas", selectionOnly?: true }` as JSON
