...
Packaged hosts read startup dependency sources from one compressed archive to reduce first-launch filesystem overhead. The loading UI starts first; the temporary source loader is released after agent initialization. Original files remain available for later imports and as a fallback.
