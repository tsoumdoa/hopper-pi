...
export const DocumentParameters = Type.Object({
	lifecycleInstanceId: Type.Optional(Type.String({ description: "Shared host process from listRhinoTargets, for new/open. Defaults to the selected process." })),
	...
	expectedActiveDocument: Type.Optional(Type.Union([Type.String(), Type.Null()], { description: "Observed active handle, or explicit null. The shared host observes this for new/open; otherwise required for new/open/activate." })),
	...
	}), { description: "The shared host observes this for new/open. Otherwise include every replaced document with its fresh token and unsaved policy; [] if none." })),
...
export function createDocumentTool(kind: DocumentKind, execute?: (id: string, request: DocumentRequest) => Promise<import("@earendil-works/pi-coding-agent").AgentToolResult<unknown>>) {
	return defineTool({
		...
		async execute(_id, params) {
			if (execute) return execute(_id, params as DocumentRequest);
			return executeDocumentTool(kind, params as DocumentRequest);
		},
	});
}

export async function executeDocumentTool(kind: DocumentKind, request: DocumentRequest) {
			try {
				const result = await manageDocument(kind, request);
