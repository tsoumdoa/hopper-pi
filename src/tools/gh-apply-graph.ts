...
export const ghApplyGraphTool = defineTool({
	...
	async execute(_toolCallId, params, _signal, onUpdate) {
		...
		if (result.outcome !== "succeeded") {
			return {
				content: [{ type: "text", text: result.data && typeof result.data === "object" && "timedOut" in result.data
					? formatApplyGraphResult({ ok: false, ...(result.data as any) })
					: formatCoreFailure(result) }],
