...
if (args.includes("--help")) {
	console.log("Usage: node scripts/benchmark-host-startup.mjs --entry <packaged host/index.js> [--node <node executable>] [--runs 3] [--timeout-ms 60000] [--browser-handshake] [--output report.json]");
...
for (let index = 0; index < args.length; index++) {
	if (args[index] === "--browser-handshake") continue;
	if (!["--entry", "--node", "--runs", "--timeout-ms", "--output"].includes(args[index])) throw new Error(`Unknown option: ${args[index]}`);
	option(args[index]);
	index++;
}
const browserHandshake = args.includes("--browser-handshake");
const WebSocket = browserHandshake ? (await import("ws")).WebSocket : undefined;
...
`);
		...
		const result = { run, listeningMs: null, firstHealthMs: null, loadingPageMs: null, browserAuthSentMs: null, readyMs: null, runtimeModulesMs: null, stages: [] };
		let stderr = "";
		let pending = "";
		let exitCode;
		child = spawn(executable, ["--import", pathToFileURL(preload).href, entry, ...(browserHandshake ? ["--wait-for-browser"] : []), "--explicit-start", "--data-dir", data,
		...
		while (elapsed() < timeout) {
			...
			if (discovery) {
				...
				if (response && response.pid === child.pid && response.hostEpoch === discovery.hostEpoch) {
					...
					if (browserHandshake && !socket) {
						const ack = await http(discovery.endpointPort, "/api/shared/browser-ready", "POST", {
							authorization: `Bearer ${discovery.registrationToken}`, "X-Hopper-Host-Epoch": discovery.hostEpoch,
						});
						if (!ack || ack.status !== 204) throw new Error("Packaged host does not support the browser startup handshake");
						const page = await http(discovery.endpointPort, "/");
						if (!page || page.status !== 200) throw new Error("Loading page was not served during startup");
						result.loadingPageMs = elapsed();
						const state = JSON.parse(await readFile(join(home, ".hopper", "shared-control", "control.json"), "utf8"));
						socket = new WebSocket(`ws://127.0.0.1:${discovery.endpointPort}/ws-shared`, { origin: `http://127.0.0.1:${discovery.endpointPort}` });
						socket.on("error", () => {});
						socket.once("open", () => {
							socket.send(JSON.stringify({ type: "authenticate", token: state.browserCredential }));
							result.browserAuthSentMs = elapsed();
						});
					}
...
const report = { entry, executable, platform: process.platform, browserHandshake, recordedAt: new Date().toISOString(),
