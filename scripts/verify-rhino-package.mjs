...
export async function verifyRhinoPackage(options) {
	...
	for (const file of files) {
		...
		if ((file.relativePath === "Hopper.Grasshopper.gha"
			|| file.relativePath === "Hopper.Rhino.rhp")
			&& (!binary || binary.format !== "pe" || !binary.managed)) {
			errors.push(`${file.relativePath}: required plug-in must be a managed PE assembly`);
			continue;
		}
