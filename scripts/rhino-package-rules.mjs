...
export const PACKAGE_ALLOW_RULES = Object.freeze([
	...
	},
	{
		...
		description: "managed plug-in files and Yak metadata live at the package root",
		test: (path) => !path.includes("/") && (
			/^(manifest\.yml|LICENSE(?:\.[A-Za-z0-9_-]+)?|rhino-package-manifest\.json)$/i.test(path)
			|| /\.(dll|rhp|gha|json|config)$/i.test(path)
		),
	},
	{
		id: "runtime-manifest",
