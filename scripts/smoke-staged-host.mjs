...
const smokeSource = [
	...
	`const compilerLoaded = () => Object.keys(require.cache).some(path => path.replaceAll("\\\\", "/").includes("/jiti/dist/"));`,
	`if (compilerLoaded()) throw new Error("Pi SDK eagerly loaded the extension compiler");`,
	`const { Type } = await import("typebox");`,
	`const { Type: piType } = await import("@earendil-works/pi-ai");`,
	`if (Type.Object !== piType.Object) throw new Error("Pi and extensions must share the bundled schema implementation");`,
	`const core = await import("@earendil-works/pi-agent-core");`,
	`const coreNode = await import("@earendil-works/pi-agent-core/node");`,
	`if (core.Agent !== coreNode.Agent) throw new Error("Agent entrypoints must share bundled module state");`,
	...
	` if (compilerLoaded()) throw new Error("Inline Hopper extensions loaded the extension compiler");`,
	` const extension = join(temporary, "extension.ts");`,
	` await writeFile(extension, 'import type { ExtensionAPI } from "@earendil-works/pi-coding-agent"; import { Type } from "typebox"; import { Type as PiType } from "@earendil-works/pi-ai"; if (Type.Object !== PiType.Object) throw new Error("Extension loaded a duplicate schema implementation"); export default (pi: ExtensionAPI) => { pi.registerTool({ name: "staged_smoke", label: "Smoke", description: "Smoke", parameters: Type.Object({}), execute: async () => ({ content: [{ type: "text", text: "ok" }], details: {} }) }); };');`,
	` const bundledLoaded = await pi.discoverAndLoadExtensions([extension], temporary, join(temporary, "extension-agent"));`,
	` if (bundledLoaded.errors.length || !bundledLoaded.extensions[0]?.tools.has("staged_smoke") || !compilerLoaded()) throw new Error("Bundled SDK did not load the typed extension on demand: " + JSON.stringify(bundledLoaded.errors));`,
