...
try {
    ...
    $stage = Join-Path $PWD "artifacts\hopper-pi-$version-win-x64-local-$([Guid]::NewGuid().ToString('N'))"
    New-Item -ItemType Directory -Path $stage | Out-Null

    Write-Host '[hopper-pi] Installing JavaScript dependencies'
    ...
    if ($BuildOnly) {
        ...
        $existing = @($installed | Where-Object { $_ -match '^\s*hopper-pi\s+\(' })
        if ($existing.Count -gt 0 -and -not $Yes) {
            if ([Console]::IsInputRedirected) { throw 'hopper-pi is already installed. Rerun with -Yes to replace it.' }
            $reply = Read-Host "$($existing -join ', ') is installed. Replace it? [y/N]"
            if ($reply -notmatch '^(?i:y|yes)$') {
                Write-Host "[hopper-pi] Installation cancelled. Package files: $stage"
                return
            }
        }
        Assert-RhinoClosed
        Invoke-Checked node @('scripts/stop-shared-host.mjs')
        if ($existing.Count -gt 0) { Invoke-Checked $yak @('uninstall', 'hopper-pi') }
        Invoke-Checked $yak @('install', "--source=$stage", 'hopper-pi', $version)
        $installed = @(Invoke-Checked $yak @('list'))
        $expected = '^\s*hopper-pi\s+\(' + [regex]::Escape($version) + '\)'
        if (-not ($installed -match $expected)) { throw "Yak did not report hopper-pi $version as installed." }
        Write-Host "[hopper-pi] Installed hopper-pi $version. Package files: $stage"
        Write-Host '[hopper-pi] In Rhino, run HopperCode to start the new host and open the browser UI.'
        if ($OpenRhino) {
