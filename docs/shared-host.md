...
Reloading the browser or opening HopperCode in another connected document restores the selected conversation within the current host session. If the browser has no saved selection, the journal supplies the latest active or previous thread from that session. New chat explicitly creates a conversation. See [persistence and browser reconnects](#persistence-and-browser-reconnects) for session boundaries after host restart.
...
Import enumerates the native object table explicitly and rejects missing geometry or attributes before mutation.
