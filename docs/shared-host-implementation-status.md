...
The normal browser UI provides conversations, process/document selection, conversation replies, steering, questions, recovery acknowledgements, bounded document and launch controls, model selection and authentication. Task execution continues without a connected browser. A replacement tab takes control without cancelling tasks.
...
The normal conversation view excludes diagnostic task history, internal artifact dumps, and acceptance-test entries. Diagnostic evidence remains available in the journal/export. A compact document picker lists available models with friendly names; it excludes detached instances while preserving an unavailable warning for a previously selected document. Fixture-only conversations are omitted, and real user messages within a former fixture conversation remain visible.

UI cleanup validation passed `pnpm build` and 637 TypeScript tests across 75 files. Desktop and mobile browser checks include a 32-document picker. A read-only copy of the local history verified that eight diagnostic tasks and seven fixture-only conversations disappear from the browser projection while all three real user tasks remain byte-for-byte unchanged; no live database mutation was performed. An independent review checked fixture classification, preserved steering/image messages, and multi-instance target labels.

## Connection and target regression follow-up

Concurrent Rhino startup now waits for an initializing owner to publish matching discovery and health on the pinned endpoint. The wait uses the startup deadline and observes changes to stopped intent. An unrelated or hung owner still blocks startup, and the waiting launcher never starts a fallback host. Previously, a second Rhino could fail after the first host bound HTTP but before it published discovery.

Native browser opening now consumes one explicit HopperCode request. Background reattachments update readiness without opening extra tabs that would replace the browser controller. The normal web application ignores stale socket events, bounds authentication, probes silent connection loss, and reacts to online/page restoration. It retains the current conversation and retries pending durable commands with their captured targets and original request IDs. Replaced tabs and rejected credentials require explicit user action. Vite resolves the shared endpoint for each API request and WebSocket connection so starting Vite before the host does not permanently cache an unavailable endpoint.

The composer retains its model picker and adds a target-settings dialog for selecting up to 16 Rhino or Grasshopper documents, authorizing one new/open document action, or authorizing a verified Rhino launch. Save/discard policy remains explicit. Additional Mac process launch stays unavailable while a Rhino process is attached. These permissions are captured with the submission, retained on transport failure or rejection, and cleared after acceptance. Ambiguous initial inventories require a selection; a disconnected selection never silently switches to another model.

Validation passed 683 TypeScript/UI tests across 77 files, 247 C# Core tests, the production host/web build, the Rhino plugin build, and `git diff --check`. New coverage includes simultaneous startup during initialization, Stop during that wait, Vite-before-host API/WebSocket discovery, browser takeover, silent socket loss, stale acknowledgements, captured multi-target retries, new/open grants, launch eligibility, and Grasshopper selection. No connected browser was available for visual verification. Packaged Rhino, native sleep/wake, and Windows acceptance were not repeated for this follow-up.
