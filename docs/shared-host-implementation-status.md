...
Closing the browser or Rhino leaves the host running. Reloading or reopening the browser restores the selected conversation, including running tasks and pending questions. If the browser has no saved selection, the journal supplies the latest active or previous thread. New chat explicitly creates a conversation. Stopping or restarting the host preserves history, but possibly started work becomes interrupted or uncertain and is not automatically replayed.
