...
A separate `tool-settings.initialized` marker is written before initial settings creation. Missing settings in an initialized profile require explicit repair. Invalid or future schemas also block execution. Adding a tool appends its default preference and advances the revision under the lock; existing choices, epoch, and credential reference are preserved. Removed tools retain their stored preferences for other hosts and later reinstalls. Every stored gate is validated, including retired IDs. Repair preserves the damaged settings in a uniquely named backup, starts a new epoch with Firecrawl disabled, and requires credential setup again. It never adopts an old protected entry.
