...
macOS stores keys in Keychain and Windows uses Credential Manager through the pinned `@napi-rs/keyring` native binding. Linux uses Secret Service through libsecret's `secret-tool` utility. Install that utility and unlock the desktop credential store before setup. Hopper passes keys through stdin, not command arguments. Missing or locked protected storage blocks Firecrawl; there is no plaintext fallback. Linux deliberately avoids the binding's automatic fallback to a nonpersistent kernel keyring.
