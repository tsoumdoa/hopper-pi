...
const defaults = (): AgentToolsSnapshot => ({
	tools: [
		{ id: "hopper.tool.rh_document", name: "rh_document", parent: "hopper.rhino", description: "Inspect and manage the Rhino document.", enabled: true, active: true, available: true, status: "active", parameters: { type: "object", properties: { action: { type: "string", enum: ["get", "open", "save"] } } } },
		{ id: "hopper.tool.gh_list_components", name: "gh_list_components", parent: "hopper.grasshopper", description: "Search the Grasshopper component library.", enabled: true, active: true, available: true, status: "active", parameters: { type: "object" } },
...
export function createMockToolSettings() {
	let snapshot = defaults();

	/** Shared dev fixture. Discards submitted keys; never uses protected storage or a provider. */
	return function mockToolSettings(action?: ToolSettingsAction): AgentToolsSnapshot | ToolSettingsResult {
		...
		if (action.type === "reset" || action.type === "repair") {
			...
			if (action.type === "patch") {
				...
				if (!gate) return { ok: false, code: "error", error: "Unknown tool or group", snapshot: structuredClone(snapshot) };
				gate.enabled = action.patch.enabled;
			} else if (action.type === "credential") {
				const parent = settings.parents.find(parent => parent.id === action.pluginId);
				if (!parent?.credential) return { ok: false, code: "error", error: "Unknown plugin credential", snapshot: structuredClone(snapshot) };
...
export const mockToolSettings = createMockToolSettings();
