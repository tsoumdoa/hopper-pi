import { ArrowDown, Brain, ChevronRight, CircleAlert, CircleCheck, Loader2, Sparkles, Wrench } from "lucide-react";
...
function ToolCard({ tool }: { tool: ToolCall }) {
	...
	return (
		<Collapsible
			...
			className={cn(
				"overflow-hidden rounded-lg border bg-surface text-xs transition-colors",
				tool.status === "error" ? "border-danger/30" : "border-line hover:border-line-strong",
			)}
		>
			<CollapsibleTrigger className="group flex w-full items-center gap-2 px-2.5 py-2 text-left outline-none focus-visible:bg-surface-muted">
			...
			<CollapsibleContent className="border-t border-line bg-surface-muted">
				{hasResult && (
					<div className="border-b border-line px-2.5 py-2">
						<p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-muted">Input</p>
				...
				<div className="px-2.5 py-2">
					{hasResult && <p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-muted">{tool.status === "error" ? "Error" : "Output"}</p>}
...
function ThinkingBlock({ text, streaming }: { text: string; streaming: boolean }) {
	return (
		<Collapsible className="text-xs">
			<CollapsibleTrigger className="group inline-flex items-center gap-1.5 rounded-md py-1 text-muted outline-none transition-colors hover:text-ink focus-visible:ring-2 focus-visible:ring-accent/35">
	...
	return (
		<article className="flex justify-end animate-slide-up" aria-label="Your message">
			<div className="max-w-[min(85%,560px)]">
				...
				<div className="whitespace-pre-wrap break-words rounded-2xl rounded-br-md bg-accent px-4 py-2.5 text-[15px] leading-6 text-white shadow-sm">{message.text}</div>
			</div>
		</article>
	);
}
...
function AssistantMessage({ message }: { message: ConversationMessage }) {
	...
	return (
		<article className="flex gap-3 animate-slide-up" aria-label="Hopper's reply">
			<span aria-hidden="true" className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-lg bg-accent-soft text-accent">
				<Sparkles className="size-3.5" />
			</span>
			<div className="min-w-0 flex-1 pt-0.5">
				<div className="mb-1 flex items-center gap-2 text-xs">
					<span className="font-semibold text-ink">Hopper</span>
				...
				{message.text && (
					<div className="whitespace-pre-wrap break-words text-[15px] leading-7 text-ink">
						{message.text}
				...
				{message.error && (
					<p className="mt-2 flex items-start gap-2 rounded-lg border border-danger/30 bg-danger/5 px-3 py-2 text-sm leading-5 text-danger" role="alert">
...
function Welcome({ connected, onSuggestion }: { connected: boolean; onSuggestion(prompt: string): void }) {
	return (
		<section className="mx-auto mt-[max(6vh,1rem)] max-w-[600px] animate-slide-up" aria-labelledby="welcome-title">
			<div className="rounded-2xl border border-line bg-surface p-7 shadow-card sm:p-9">
				<p className="text-[11px] font-semibold uppercase tracking-[.12em] text-accent">{connected ? "Ready when Rhino is" : "Connecting to Rhino"}</p>
				<h2 id="welcome-title" className="mt-3 text-[clamp(24px,3.4vw,34px)] font-semibold leading-[1.15] tracking-[-.02em]">
					Build in Grasshopper by describing what you need.
				</h2>
				<p className="mt-3 max-w-[480px] text-sm leading-relaxed text-ink-soft">
					Hopper inspects the active canvas, adds and wires components, runs Rhino scripts, and shows each tool call as it works.
				</p>
				<div className="mt-6 flex flex-wrap gap-2" role="group" aria-label="Prompt suggestions">
...
export function Conversation({
	...
	return (
		<div className="relative min-h-0 flex-1">
			<div ref={scroller} onScroll={onScroll} className="h-full overflow-y-auto px-4 py-6 sm:px-6 lg:px-10" aria-label="Conversation" aria-live="polite">
				<div className="mx-auto flex w-full max-w-[780px] flex-col gap-7 pb-4">
					{messages.length === 0 ? (
			...
			{showJump && (
				<Button
					size="sm"
					variant="secondary"
					className="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full shadow-pop animate-pop-in"
					onClick={() => {
