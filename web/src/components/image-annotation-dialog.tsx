...
class EditorBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
	...
	render() {
		return this.state.failed ? <p role="alert">The image editor could not load. You can still send this attachment.</p> : this.props.children;
	}
}
	...
	return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
		<DialogContent className="h-[calc(100dvh-2rem)] w-[calc(100vw-2rem)] max-w-[1400px] gap-3 overflow-hidden p-3 sm:p-4"
			...
			<DialogHeader>
				...
				<DialogDescription>Add arrows, shapes, text, or freehand marks. Save to update the attachment.</DialogDescription>
			</DialogHeader>
			<EditorBoundary>
				<Suspense fallback={<p role="status" className="flex-1 p-6 text-muted">Loading image editor…</p>}>
