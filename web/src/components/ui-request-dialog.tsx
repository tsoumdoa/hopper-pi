...
export function UiRequestDialog({
	...
	if (!request || (suppressAuth && request.kind === "auth")) return null;
	return (
		<RequestDialog
			request={request}
			queued={queued}
...
export function RequestDialog({
		...
		<>
			{inline ? (
				...
				<DialogHeader>
					...
					{request.description && kind !== "confirm" && (
			...
			>
				...
				<DialogFooter>
					...
					<Button type="submit" disabled={kind === "select" && !value}>
						{kind === "confirm" ? "Confirm" : "Continue"}
					</Button>
				</DialogFooter>
			</form>
		</>
	...
	return inline ? (
		<div className="grid gap-3 rounded-md border border-line p-3">{content}</div>
	) : (
		<Dialog
			open
			onOpenChange={(open) => {
		...
		>
			<DialogContent hideClose>{content}</DialogContent>
		</Dialog>
	);
}
